...
enum class BehaviorProperties {
}

enum class WindowTile {
    single,
    left, middle, right,
    up_left, up, up_right,
...
enum class MiniCubeMaskState {
    ...
    val mask: Int
        get() = name.substring(1).toInt()

    companion object {
        fun fromMask(mask: Int): MiniCubeMaskState {
            return valueOf("c${mask.coerceIn(1, 255)}")
        }
    }
}
